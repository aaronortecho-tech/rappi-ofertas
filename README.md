...
- Se revisa cada 30 minutos, pero avisa en un **resumen cada 3 horas** (variable `HORAS_RESUMEN_HOGAR`). Las ofertas de 80 % o más no esperan: llegan en la misma ronda. Las demás esperan en una cola guardada en la memoria; si un producto reaparece con otro precio, queda el último, y lo que no se vuelve a ver en 24 horas sale de la cola. Cada resumen tiene como máximo 8 mensajes de hasta 3 productos; no marca como enviadas las ofertas que quedaron fuera del límite. No repite una misma ficha/precio/condición durante 7 días. Un cambio de precio puede generar un aviso nuevo.
