...
- Cola de hogar: `home_queue_selection` conserva el reparto de 300 cupos normales por categoría. Primero conserva bajadas con evidencia histórica; reserva hasta un tercio del cupo de cada categoría (mínimo uno si hay espacio) para observaciones de la ronda y completa por puntaje. Los urgentes siguen fuera de ese límite. `recientes_rescatadas` cuenta candidatas conservadas que el recorte anterior habría eliminado, no ofertas necesariamente enviadas. `observadas_ronda_enviadas` mide entregas observadas en la ronda, incluidas las que no necesitaron la reserva.
- Reloj del resumen: solo avanza si se entregó al menos una oferta normal y no hubo fallos de envío. Enviar únicamente urgentes, simular o fallar no pospone el resumen normal. `deliver` puede devolver el detalle de entregas exitosas mediante `delivered`; las métricas separan `normales_enviadas` y `urgentes_enviadas`. Se mantienen 24 confirmadas y los presupuestos de ocho páginas/16 consultas.
